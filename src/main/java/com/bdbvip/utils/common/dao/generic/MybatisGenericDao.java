package com.bdbvip.utils.common.dao.generic;
...
abstract public class MybatisGenericDao
    ...
    //返回的是插入成功的条数
    public Serializable save(String statement, Object o) {
    	return getSqlSession().insert(statement, o);
    }
    ...
    public <T> List<T> list(String statement, Object parameter, int size) {
        return (List<T>) getSqlSession().selectList(statement, parameter, new RowBounds(0, size));
    }
    ...
    public Page pagedQuery(String count, String statement, Object parameter,
        ...
        list = getSqlSession().selectList(statement, parameter, new RowBounds(startIndex,
                pageSize));
        Page resultPage = new Page(startIndex, totalCount, pageSize, list);
        resultPage.setCurrentPageNo(pageNo);
        return resultPage;
    }
	...
	public <T>T getMapper(Class<T> clazz) {
		return this.getSqlSession().getMapper(clazz);
	}
}
