...
public class CookieUtil {
	...
	public static String getArea(String servletURL) {
		try {
			...
			if (d.length > 0){
				...
				if (domain.indexOf(".ecp888.")!=-1){
					//判断是否为.ecp888.com的域名
					return domain.substring(domain.indexOf(".ecp"), domain.length());
				}else{
					return domain;
				}
			}else{
	...
	public  static String getUserId(HttpServletRequest request) {
		Object cookieValue = getCookieValue(request, CommUtil.LOGIN_TOKEN_CODE);
		if (cookieValue == null || cookieValue.equals("")) {
			return "-1";
		} else {
